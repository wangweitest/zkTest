 ...
 */
public class ZKConnect1 implements Watcher {
		
	final static Logger log = LoggerFactory.getLogger(ZKConnect1.class);
...
//	public static final String zkServerPath = "192.168.1.110:2181";
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * watcher：通知事件，如果有对应的事件触发，则会收到通知；如果不要，那就设置为null
		 * canBeReadOnly：可读，当这个物理机节点断开后，还是可以读到数据的，只是不能写，
		 * 					       此时数据被读取到的可能是旧数据，此处建议设置为false，不推荐使用
		 * sessionId：会话的id
		 * sessionPasswd：会话密重连当会话丢失后，可以依据 sessionId 和 sessionPasswd 重新获取会话
		 */
		
		//创建一个zk客户端
		ZooKeeper zk = new ZooKeeper(zkServerPath, timeout, new ZKConnect1());
		
		log.info("客户端开始连接zookeeper服务...");
		log.info("连接状状态：{}" ,zk.getState());
